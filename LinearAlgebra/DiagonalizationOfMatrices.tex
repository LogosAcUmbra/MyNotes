...
\begin{document}
    ...
    \subsection{Vector Set vs Vector Space}
    We use different notations on vector sets and vector spaces: 
    \begin{alignat*}{2}
        \textbf{ sets } &
            & \textbf{ spaces } & \\[1em]
        \mathbb{R}^n&: \text{ the set of all real vectors } 
            & \myvecsp{V}&: \text{ the whole real vector space } \\
        \myset{B} = \mathbb{R}^n&: \text{ set B is the entire vector space } 
            & \myset{C} = \myvecsp{V}&: \text{ (same) set C is the entire vector space } \\
        \myset{B} \subset \mathbb{R}^n&: \text{ set B is a proper subset of }\mathbb{R}^n
            & \myset{C} < \myvecsp{V}&: \text{ set C is a proper subspace of }\myvecsp{V} \\
        \myset{B} \subseteq \mathbb{R}^n&: \text{ set B is a set of real vectors } 
            & \myset{C} \le \myvecsp{V}&: \text{ set C is a vector space } \\
        \myset{B} \nsubseteq \mathbb{R}^n&: \text{ set B contains non-real vectors } 
            & \myset{C} \not\le \myvecsp{V}&: \text{ set C is a not a vector space } \\
        \myset{B} \subset \myset{P}&: \text{ set B is proper subset of set P}
            & \myset{C} < \mysubvecsp{U}&: \text{ set C is a space and is proper subspace of U } \\
        \myset{B} \subseteq \myset{P}&: \text{ set B is subset of set P} 
            & \myset{C} \le \mysubvecsp{U}&: \text{ set C is a space and is subspace of U } \\
        \myset{B} \nsubseteq \myset{P}&: \text{ set B is not a subset of set P} 
            & \myset{C} \not\le \mysubvecsp{U}&: \text{ set or space C is not a subspace of U } \\
        \myset{B} \subset \mysubvecsp{U}&: \text{ set B is proper subset of space U} 
            & & \\
        \myset{B} \subseteq \mysubvecsp{U}&: \text{ set B is subset of space U} 
            & & \\
        \myset{B} \nsubseteq \mysubvecsp{U}&: \text{ set B is not a subset of space U} 
            & & \\
    \end{alignat*}
    ...
    \begin{itemize}
        ...
        \item All vector sets are $\subseteq \mathbb{R}^n$ ($\implies$ All vector spaces are $\le \myvecsp{V}$)
    \end{itemize}

    \newpage
    \section{Change of Basis}
    \subsection*{Quick Review: From Axioms to Coordinates}
    Let $\mysubvecsp{U}$ be a vector space and $\myset{B}$ be any basis for $\mysubvecsp{U}$
    \begin{itemize}
        \item \textbf{Basis Uniqueness:} For any $\myvec{v} \in \mysubvecsp{U}$, the coordinate vector $[\myvec{v}]_\myset{B}$ is unique.
        \item \textbf{Origin Invariance:} All coordinate systems in $\mysubvecsp{U}$ share the same $\myvec{0}$ vector.
    ...
    For the change of basis $[x]_\myset{B} \to [x]_\myset{C}$ to be valid, both sets must be bases for the \textbf{same underlying vector space} $\mysubvecsp{U}$.
    \begin{equation}
        \myset{B}, \myset{C} \subset \mysubvecsp{U} \quad \text{and} \quad \operatorname{Span}(\myset{B}) = \operatorname{Span}(\myset{C}) = \mysubvecsp{U}
    \end{equation}
    This ensures that every vector $\myvec{u} \in \mysubvecsp{U}$ has a unique representation in both coordinate systems.
    ...
    \subsubsection{General Formula for Transiting in Two Bases}

    Let $\myset{B} = \{\myvec{b}_1, \dots, \myvec{b}_n\}$ and $\myset{C}$ be two bases for $\mysubvecsp{U}$.
    ...
    for all linear transformation $\mymat{L} = [\mymat{L}]_{\myset{S}}$,\\ 
    ...
    that is applicable to vectors in basis $\myset{C}$, $[\mymat{L}]_{\myset{C}}$, is given by
    \begin{equation}
    ...
    has its \textbf{direction unchanged} (or reversed, or shrunk into a point) in a linear transformation
    ...
    For a given eigenvalue $\lambda_k$, the \textbf{eigenspace} $\mysubvecsp{E}_{\lambda_k} \le \myvecsp{V}$, \\
    is the set of all eigenvectors corresponding to $\lambda_k$, plus the zero vector.
    \begin{equation}
    ...
    \begin{itemize}
        \item \textbf{Number of Eigenvalues:} \vspace{1ex} \\
            ...
            \begin{enumerate}
                ...
                \item The number of real eigenvalues $m$ is $0 \le m \le n$.
            \end{enumerate}
        ...
        \item \textbf{Dimension of Eigenspaces:} \vspace{1ex} \\
            ...
            \begin{itemize}
                \item \textbf{Real:} $\mysubvecsp{E}_{\lambda_k} \le \myvecsp{V}$ for all real $\lambda_k$.
                ...
                \end{equation}
                \item \textbf{Total Dimension:} The sum of dimensions $\sum_{k=1}^m \dim \mysubvecsp{E}_{\lambda_k}$ is in range $[m, n]$.
                \item \textbf{Special Case, All Real and Distinct:} We have: total dimension $ = n$ if $m = n$. \\ 
                    \begin{quote}
    ...
    derivation: Assume $\mymat{A}$ is invertible, let $\lambda$ and $\lambda'$ be the eigenvalues of $\mymat{A}$ and $\mymat{A}^{-1}$ respectively\\
    ...
    \begin{align*}
        ...
        \myvec{v} &= \lambda \mymat{A}^{-1} \myvec{v} & \mymat{L}(k\myvec{v}) = k\mymat{L}(\myvec{v}) \text{ for all lin. transf. function }\mymat{L}\\
    ...
    for all $n \times n$ square matrix $\mymat{A}$ with eigenvalues $\lambda_1, \lambda_2, \dots, \lambda_n$
    \begin{equation}
    ...
    Since $P(\lambda) = \det(\mymat{A} - \lambda\mymat{I})$ for all $\lambda$, we can substitute $\lambda = 0$, \vspace{1ex} \\
    ...
    \begin{equation}
        \mysubvecsp{E}_{\lambda_1} \oplus \mysubvecsp{E}_{\lambda_2} \oplus \dots \oplus \mysubvecsp{E}_{\lambda_m} = \mathbb{R}^n
    \end{equation}
    ...
    \[ \mymat{L}(\myvec{x}) = \mymat{A}\myvec{x} \qquad \text{ for all }n \times 1\text{ column vector }\myvec{x} \]
    ...
    \begin{minipage}{\textwidth}
        ...
        \begin{equation}
            \mymat{P} = \mymat{P}_{\myset{S}\leftarrow\myset{E}} = \Big[ \myvec{e}_1\,\myvec{e}_2\,\dots\,\myvec{e}_n \Big] \qquad \text{ where }\mymat{P}\text{ suits }\mymat{D} = \mymat{P}^{-1} \mymat{A} \mymat{P}
        \end{equation}
    \end{minipage}
    ...
    \end{align*}
    $\because$ for all diagonal matrix $\mymat{D} = \begin{bmatrix}
            d_{11}  & 0         & \dots     & 0 \\
            0       & d_{22}    & \dots     & 0 \\
    ...
    A set of vectors $\myset{P} = \{\myvec{p}_1, \myvec{p}_2, \dots, \myvec{p}_m\}$ is an \textbf{orthonormal set} \\
    if \textbf{all} the following requirements are satisfied
    \begin{itemize}
        \item $\myset{P}$ is orthogonal set
        \begin{itemize}
            \item $\dotprod{\myvec{p}_i}{\myvec{p}_j} = 0 \quad \forall\,i, j \in \{1, \dots, m\} \text{ with } i \neq j$
        \end{itemize}
        \item $\|\myvec{p}_k\| = 1 \quad \forall\,k \in \{1, \dots, m\}$
    \end{itemize}
    \paragraph{Note:} $\myset{P} \not\le \myvecsp{V}$

    
    \subsubsection{Orthogonal Complements}
    \paragraph{Orthogonal to a Set of Vectors}
    \begin{quote}
        a vector $\myvec{z}$ is orthogonal to a vector space $\mysubvecsp{U}$ if \\
        \[ \dotprod{\myvec{z}}{\myvec{v}} = 0 \quad \forall\,\myvec{v} \in \mysubvecsp{U} \]
    \end{quote}
    
    \paragraph{Orthogonal Complement of a Set of Vectors} 
    
    \begin{quote}
        The \textbf{orthogonal complement} of a vector space $\mysubvecsp{U}$ (denoted~by~$\mysubvecsp{U}^{\perp}$) is the set of \textbf{all}~vectors~$\myvec{z}$ that are orthogonal to $\mysubvecsp{U}$ \\
        \vspace{1ex}
        Note:
        \begin{itemize}
            \item $\mysubvecsp{U}^{\perp} < \myvecsp{V}$
            \item any $\myvec{z} \in \mysubvecsp{U}^{\perp}$ is also orthogonal to $\operatorname{Span}(\mysubvecsp{U})$, $\implies \mysubvecsp{U}^{\perp} = (\operatorname{Span}(\mysubvecsp{U}))^{\perp}$
        \end{itemize}
    \end{quote}
    ...
    by taking the columns as column vectors, all the column vectors forms an orthonormal set
    ...
    \noindent
    \begin{minipage}{\textwidth}
        3. for each eigenvalue $\lambda_k$, geometric multiplicity = algebraic multiplicity
        \begin{quote}
            TODO
        \end{quote}
    \end{minipage} 
    ...
    \\
    Take it briefly, given a set of basis vectors of any vector space $\mysubvecsp{U} \le \myvecsp{V}$,\\
    \quad the process find a set of orthogonal (but not normalized) basis for $\mysubvecsp{U}$ \vspace{1ex}
    
    It is as follows:
    \begin{quote}
        Let $\{\myvec{w}_1, \myvec{w}_2, \dots, \myvec{w}_n\}$ be a basis for any vector space $\mysubvecsp{U} \subseteq \mathbb{R}^n$, then\\
        the orthogonal basis $\{\myvec{v}_1, \myvec{v}_2, \dots, \myvec{v}_n\}$ for $\mysubvecsp{U}$ can be found as follows:
